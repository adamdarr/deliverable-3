 ...
 * then the cart subtotal should be the same as the total price of the multiple items.
 */
public class TestCartSubtotal {
  ...
  @Test
  public void testCartSubtotal() throws Exception {
    driver.get(baseUrl + "/");
    ...
    String itemsPrice = driver.findElement(By.cssSelector("div.orderItem-price-container.globalCart-plainText > span")).getText();
    String subtotal = driver.findElement(By.cssSelector("span.lineItem-val")).getText();
    
    // ERROR: Caught exception [ERROR: Unsupported command [getEval | '${itemsPrice}' === '${subtotal}' ? true : false | ]]
    // just use junit assertEquals instead
    assertEquals(itemsPrice, subtotal);
  }
